...
$stmt = $conn->prepare("
    SELECT id, title, description, public, access_code, created_by, auto_start, countdown_enabled, countdown_seconds, total_time, start_date
    FROM quizzes
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action']) && $_POST['action'] === 'update_quiz') {
    ...
    $is_public = $is_public = isset($settings['is_public']) && $settings['is_public'] ? 1 : 0;
    ...
    $stmt = $conn->prepare("
        UPDATE quizzes SET
            title = ?, description = ?, public = ?, auto_start = ?, countdown_enabled = ?, countdown_seconds = ?, total_time = ?, start_date = ?
        WHERE id = ? AND created_by = ?
    ");
    $stmt->execute([$title, $description, $is_public, $auto_start, $countdown_enabled, $countdown_seconds, $total_time, $start_date, $quiz_id, $_SESSION['user_id']]);

    $del_a_stmt = $conn->prepare("DELETE FROM answers WHERE question_id IN (SELECT id FROM questions WHERE quiz_id = ?)");
    $del_a_stmt->execute([$quiz_id]);

    $del_q_stmt = $conn->prepare("DELETE FROM questions WHERE quiz_id = ?");
    $del_q_stmt->execute([$quiz_id]);

    foreach ($questions_data as $position => $question) {
    ...
    echo json_encode(['success' => true, 'access_code' => $quiz['access_code']]);
    exit;
}
...
$loaded_data = [
    'settings' => [
        ...
        'is_public' => (bool) $quiz['public'],
        'start_date' => $quiz['start_date'],
        'auto_start' => (bool) $quiz['auto_start'],
        'countdown_enabled' => (bool) $quiz['countdown_enabled'],
...
<body>
    ...
    <script>
        ...
        let loadedQuiz = <?php echo json_encode($loaded_data); ?>;
                ...
                answersContainer.innerHTML = `
                    <div class="answer-option">
                        <div class="answer-input-with-prefix">
                            ...
                            <input type="text" placeholder="Верно" class="answer-input" value="Верно">
                        </div>
                        <div class="answer-controls">
                            <input type="radio" name="correct" value="0" checked class="correct-radio">
                    ...
                    <div class="answer-option">
                        <div class="answer-input-with-prefix">
                            ...
                            <input type="text" placeholder="Неверно" class="answer-input" value="Неверно">
                        </div>
                        <div class="answer-controls">
                            <input type="radio" name="correct" value="1" class="correct-radio">
                            <button class="remove-answer-btn" style="display: none;">
        ...
        function selectQuestion(questionId) {
            ...
            isMultipleAnswers = question.answers.filter(a => a.correct).length > 1;
            document.getElementById('multipleAnswers').checked = isMultipleAnswers;
        ...
        function updateAnswersUI(answers) {
            ...
            answers.forEach((answer, index) => {
                ...
                const showRemoveBtn = type === 'multiple_choice' && answers.length > 2 && index >= 2;
        ...
        function loadQuizData() {
            ...
            const settings = loadedQuiz.settings;
            document.getElementById('quizTitle').value = settings.title;
            document.getElementById('titleCounter').textContent = settings.title.length;
            document.getElementById('quizDescription').value = settings.description;
            document.getElementById('descCounter').textContent = settings.description.length;
            document.getElementById('isPublic').checked = settings.is_public;
            document.getElementById('startDate').value = settings.start_date || '';
            document.getElementById('autoStart').checked = settings.auto_start;
            document.getElementById('countdownEnabled').checked = settings.countdown_enabled;
            document.getElementById('countdownSeconds').value = settings.countdown_seconds;
            document.getElementById('totalTime').value = settings.total_time;

            toggleStartTimeSettings();

            currentQuestions = loadedQuiz.questions;
            ...
            currentQuestions.forEach(question => {
                ...
                questionItem.innerHTML = `
                    <div class="question-item-header">
                        <span class="question-number">Вопрос ${question.number}</span>
                        <button class="remove-question-btn" onclick="removeQuestion(${question.id}, event)">
                            <img src="assets/images/trash.svg" alt="Удалить вопрос" width="14" height="14">
                        </button>
                    </div>
                    <div class="question-preview">${question.text || 'Новый вопрос...'}</div>
                `;
                questionItem.addEventListener('click', () => selectQuestion(question.id));
                questionsList.appendChild(questionItem);
            });

            if (currentQuestions.length > 0) {
                selectQuestion(currentQuestions[0].id);
                document.getElementById('questionEditor').style.display = 'block';
                document.querySelectorAll('.question-item')[0].classList.add('active');
            } else {
                document.getElementById('questionEditor').style.display = 'none';
            }
